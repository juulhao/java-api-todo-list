...
import br.com.juliorodrigues.utils.Utils;
...
public class TaskController {
  ...
  public ResponseEntity create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
    ...
    if (currentDate.isAfter(taskModel.getStartAt()) || currentDate.isAfter(taskModel.getEndAt())) {
      return ResponseEntity.status((HttpStatus.BAD_REQUEST))
          .body("A data de início e fim da task deve ser maior do que a data atual");
    }
  ...
  public ResponseEntity update(@RequestBody TaskModel taskModel, @PathVariable UUID id, HttpServletRequest request) {
    ...
    if (!task.getIdUser().equals(userID)) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Usuário não autorizado para fazer essa alteração.");
    }
